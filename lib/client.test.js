...
const Client = require('./generic-client')
...
test('generic client test', t => {
  var generic = new Client(smaMock, API_KEY, API_SECRET, 'https://kassensichv.fiskaly.com/api/v0')
  t.truthy(generic.request(
    'GET',
    '/client',
    undefined,
    undefined, 
...
test.serial('list all TSS', async t => {
  for (let offset = 0; ; offset += 100) {
    ...
    t.truthy(list)
    const { data, count } = list.body
    if (data.some(({ _id }) => _id === tssId)) {
      return
    }
    if (offset >= count) {
      break
  ...
  t.fail('created TSS is included in the list')
})
...
test.serial('list all Clients', async t => {
  ...
  t.fail('created Client is included in the list')
})
...
test.serial('intercept Transactions', async t => {
  ...
  const txRev3 = await sdk({
    url: `/tss/${tssId}/tx/${txRev2.body.number}`,
    method: 'PUT',
    query: {
  ...
  const txRev4 = await sdk({
    url: `/tss/${tssId}/tx/${txId}`,
    method: 'PUT',
    query: {
      last_revision: txRev3.body.revision
    },
    body: {
      type: 'ORDER',
      data: {
