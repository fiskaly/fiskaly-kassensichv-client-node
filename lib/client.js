...
function createBaseClient (client, opts = {}) {
  const mergedOptions = got.mergeOptions({
    json: true,
...
function createTxInterceptingClient (client, sma) {
  ...
  const interceptTxRequest = async (options) => {
    ...
    const signedTx = await signTx(tssId, payload)
    const content = JSON.stringify(signedTx)
...
function createClient (sma, apiKey, apiSecret, baseUrl = '', opts = {}) {
  if (baseUrl !== '') {
    opts = {
      baseUrl
    }
  ...
  const txIntercepting = createTxInterceptingClient(authenticating, sma)
